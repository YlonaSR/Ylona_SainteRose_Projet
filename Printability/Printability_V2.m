...
function gridAnalysisTool()
    ...
    img_sharp = imsharpen(img, 'Radius', 15, 'Amount', 17);
    grayImage = rgb2gray(img_sharp);
    ...
    edges = edge(img_filt, 'Canny', 0.99);
    
    se = strel('line', 7, 7);
    dilatedEdges = imclose(edges, se);
    edges_clean = bwareaopen(dilatedEdges, 8000);
    img_over = grayImage;
    img_over(edges_clean) = 0;
    
    cc = bwlabel(img_over);
    stats = regionprops(cc, 'Area', 'Perimeter', 'BoundingBox', 'Centroid', 'Solidity');
    
    centroids = vertcat(stats.Centroid);
    imgCenter = [size(img_over, 2)/2, size(img_over, 1)/2];
    distances = zeros(length(centroids), 1);
    for i = 1:length(centroids)
        distances(i) = norm(centroids(i,:) - imgCenter);
    end
    [~, squareIndex] = min(distances);
    
    se = strel('disk', 7);
    squareMask = cc == squareIndex;
    squareMask = imfill(squareMask, 'holes');
    squareMask = imdilate(squareMask, se);
    squareMask = imclose(squareMask, se);
    
    % Detect scale
    scale = input('What is the scale of all images (µm) : ');
    edges = edge(img_over, 'Canny');
...
function printability = analyzePrintability(squareMask, pixel2microm, img)
    ...
    imshow(img);
    hold on;
    % Draw boudaries of inside square 
...
function analyzeSp(filenames, pathname, pixel2microm)
    ...
    for file_idx = 1:length(filenames)
        ...
        filaments_sp = (1:length(list_sp))'; % Création des numéros de pore
...
function analyzeCompareArea(filenames, pathname, pixel2microm, squareMask)
    ...
    for file_idx = 1:length(filenames)
        ...
        airePixels = props.Area;
        aire_microns = airePixels * (pixel2microm^2);
        real_square_area = aire_microns / 1000000; % Convert in mm²
